            ...
            ZStack {
                if !hasRegion {
                    NavigationLink(destination: detailView, isActive: $showDetailView, label: { EmptyView() })
                        .hidden()
                    MapView3(annotationItems: geocaches.map { geocache in MapTileAnnotation(coords: CLLocationCoordinate2D(latitude: geocache.postedCoordinates.latitude, longitude: geocache.postedCoordinates.longitude), data: geocache) }, z: $z, center: $center, userLocation: $location, userHeading: $heading) { annot in
...
struct GeocacheMapView2_Previews: PreviewProvider {
    static var previews: some View {
        GeocacheMapView2(geocaches: .constant([]))
    }
}
